...
#   NOTE: Test the script below, to other directory. not in this directory to avoid duplicate
#+  git repositories. If you don't understand this, talk to me before execting task (Gulraiz)    

#+  1) The script exits if it is not a root user
#+  2) The script is run inside a directory which is to tested.
#+  3) The script test if the current directory is already a git repository, it exits.
